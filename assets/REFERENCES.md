...
An example of AutoMod (Discord's Native Moderation Solution) failing to detect "loser" because of an exclamation mark:

![AutoMod fails with simple obfuscation](assets/documents/images/auto_mod_fail.png)
...
Seer resolves those problems by abstracting complex versatility handling to the algorithm itself and allowing moderators to configure with layman rules.

## Demonstration

[![Demonstration Video](assets/demonstration_thumbnail.png)](https://www.youtube.com/watch?v=skJ_CrrmTjw)
...
Most common approach in moderation bots is to check a message against a list of words using conventional pattern matching techniques. Even when these algorithms are optimized, they face a fundamental scalability barrier.
...
### 1. Solving Multi-Pattern Matching with Finite Automata
...
- When a violation is flagged, an analysis message message will be sent to a moderation log channel.

![Violation Log Interface](assets/documents/images/violation_log.png)

- Moderators can perform a series of action directly on the violation log to streamline the moderation process.

![Violation Log Actions Interface](assets/documents/images/violation_action.png)

- Upon a successful moderation action, the violation log will be marked as completed, enabling a clean moderation trail.

![Violation Log Completion](assets/documents/images/violation_completion.png)

## Methodology

The challenge of modern community moderation lies in the trade-off between versatility—catching clever obfuscations—and performance—maintaining sub-millisecond response times on high-volume servers.

Seer accoplishes both goals thorugh a multi-stage architecture designed to reserve resource-intensive vuersatility evaluations for viable candidates without introducing false negatives.     
...
graph TD
    ...
    Pigeonhole -- "High-probability candidate" --> Dict["<b>Stage 3: NLP Filter </b><br>Filters out safe, innocent words"]
...
To achieve true scalability, Seer moves away from traditional, backtracking-heavy regular expressions. Instead, the engine utilizes a custom implementation of the **Aho-Corasick algorithm** to simulate a Deterministic Finite Automaton (DFA).

This approach guarantees $O(N)$ time complexity, where $N$ is the length of the message, regardless of the number of banned terms. By merging all banned terms into a single, unified trie-based state machine, Seer can simulataneously identify banned term matches in a single pass. 
...
The pigeonhole theorem states that if we allow a maximum edit distance of $k$, and partition a pattern into $k+1$ segments, then any approximate match within that distance *must* contain at least one segment that matches the original pattern exactly.
...
Resilient models often suffer from a common issue "False Positives". In the context of our problem, flagging innocent words that contain substrings of banned terms are one of them.

To resolve this, Seer employs NLP techniques like Normalization, Tokenization and Stemming to enable Lexicon Based Analysis.

### Stage 4: Levenshtein Verification

The final stage provides the definitive verdict that confirms the obfuscation attempt through the Levenshtein Edit Distance algorithm.
